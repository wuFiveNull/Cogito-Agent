...
def _build_sections(cfg: dict[str, str]) -> list[dict[str, object]]:
    ...
    sections.append(
        {
            ...
            "rows": [
                ...
                {
                    "key": "Base URL",
                    "value": redact_html(cfg.get("model.base_url", "") or "(default)"),
                },
    ...
    rows.append({"key": "Quiet Hours End", "value": cfg.get("autonomy.quiet_hours.end", "08:00")})
    rows.append(
        {"key": "Quiet Hours Timezone", "value": cfg.get("autonomy.quiet_hours.timezone", "local")}
    )
    rows.append({"key": "Daily Quota", "value": cfg.get("autonomy.notification.daily_quota", "5")})
    rows.append(
    ...
    rows.append({"key": "Feedback Enabled", "value": cfg.get("autonomy.feedback.enabled", "true")})
    sections.append(
        {
            "title": "Autonomy",
            "rows": rows,
