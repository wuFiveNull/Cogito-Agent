...
def _service(request: Request) -> MCPApplicationService:
    ...
    return MCPApplicationService(
        MCPServerManager(CapabilityRegistry(), trust_store=MCPTrustStore(db)),
        AuditLogger(db),
        SqliteMCPCallReader(db),
    )


def _present_servers(request: Request) -> list[dict[str, object]]:
    result: list[dict[str, object]] = []
    for raw_server in _service(request).list_servers():
        ...
        server["recent_calls"] = [
            ...
            for call in _service(request).list_recent_calls(str(raw_server.get("name", "")))
        ]
...
async def mcp_page(request: Request) -> HTMLResponse:
    context: dict[str, object] = {
        ...
        "version": APP_VERSION,
        "menu": menu_items(),
        "servers": _present_servers(request),
    }
...
async def mcp_trust(request: Request, name: str) -> Response:
    _service(request).trust(name, workspace_id=WORKSPACE_ID)
    return RedirectResponse("/console/mcp", status_code=303)


@mcp_router.post("/{name}/block", include_in_schema=False)
async def mcp_block(request: Request, name: str) -> Response:
    _service(request).block(name, workspace_id=WORKSPACE_ID)
    return RedirectResponse("/console/mcp", status_code=303)
...
async def mcp_revoke(request: Request, name: str, tool_name: str) -> Response:
    _service(request).revoke(name, tool_name, workspace_id=WORKSPACE_ID)
    return RedirectResponse("/console/mcp", status_code=303)
