...
def test_list_all_workspaces(decision_store: DecisionStore):
    decision_store.save_decision(
        decision_id="d1", event_id="e1", workspace_id="ws-1",
        user_id="", action="push", reason_code="ok", reason="",
        cost_score=0, priority_score=0, dedup_hit=False,
        quiet_hours_hit=False, quota_hit=False, requires_approval=False,
    )
    decision_store.save_decision(
        decision_id="d2", event_id="e2", workspace_id="ws-2",
        user_id="", action="skip", reason_code="quiet_hours", reason="",
        cost_score=0, priority_score=0, dedup_hit=False,
        quiet_hours_hit=False, quota_hit=False, requires_approval=False,
    )
...
def test_get_decision(decision_store: DecisionStore):
    decision_store.save_decision(
        decision_id="d-get", event_id="e-get", workspace_id="ws-1",
        user_id="", action="push", reason_code="ok", reason="test",
        cost_score=2.5, priority_score=3.0, dedup_hit=True,
        quiet_hours_hit=False, quota_hit=False, requires_approval=False,
    )
...
def test_count_push_recent(decision_store: DecisionStore):
    ...
    decision_store.save_decision(
        decision_id="d-count", event_id="e-count", workspace_id="ws-1",
        user_id="", action="push", reason_code="ok", reason="",
        cost_score=0, priority_score=0, dedup_hit=False,
        quiet_hours_hit=False, quota_hit=False, requires_approval=False,
    )
...
def test_save_requires_approval(decision_store: DecisionStore):
    decision_store.save_decision(
        decision_id="d-approve", event_id="e-approve", workspace_id="ws-1",
        user_id="", action="require_approval", reason_code="needs_ok", reason="",
        cost_score=0, priority_score=0, dedup_hit=False,
        quiet_hours_hit=False, quota_hit=False, requires_approval=True,
    )
