...
def test_list_pending(outbox: Outbox):
    outbox.enqueue(event_id="e1", decision_id="d1",
                   title="pending1", workspace_id="ws-1")
    outbox.enqueue(event_id="e2", decision_id="d2",
                   title="pending2", workspace_id="ws-1")
    pending = outbox.list_pending(workspace_id="ws-1")
    assert len(pending) == 2


def test_mark_sent(outbox: Outbox):
    mid = outbox.enqueue(event_id="e1", decision_id="d1",
                         title="mark sent", workspace_id="ws-1")
...
def test_mark_failed(outbox: Outbox):
    mid = outbox.enqueue(event_id="e1", decision_id="d1",
                         title="mark failed", workspace_id="ws-1")
...
def test_list_all(outbox: Outbox):
    outbox.enqueue(event_id="e1", decision_id="d1",
                   title="all1", workspace_id="ws-1")
    outbox.enqueue(event_id="e2", decision_id="d2",
                   title="all2", workspace_id="ws-1")
