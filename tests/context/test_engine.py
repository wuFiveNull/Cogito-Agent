...
def test_static_budget_defaults() -> None:
    from cogito_agent.context.engine import _DEFAULT_BUDGET_SHARES

    assert sum(_DEFAULT_BUDGET_SHARES.values()) == 1.0
    assert "recent_messages" in _DEFAULT_BUDGET_SHARES
    assert "retrieved_memory" in _DEFAULT_BUDGET_SHARES
    assert "system" in _DEFAULT_BUDGET_SHARES
    assert "memory_file" in _DEFAULT_BUDGET_SHARES
    assert "tool_file_context" in _DEFAULT_BUDGET_SHARES
    assert "response_reserve" in _DEFAULT_BUDGET_SHARES
