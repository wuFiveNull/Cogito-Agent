...
from cogito_agent.embedding.interface import EmbeddingHealth
...
def _add_memory(
    ...
    row = db.connection.execute("SELECT rowid FROM memories WHERE id = ?", (mid,)).fetchone()
    if row:
        db.connection.execute(
            "INSERT OR IGNORE INTO memories_fts (rowid, text, summary) VALUES (?, ?, ?)",
            (row[0], text, text[:100]),
        )
    db.connection.commit()
...
class TestDenseMemoryRetriever:
    def test_dense_retrieval_finds_semantic_match(self, db):
        ...
        vec = provider.embed_text("出差产生的交通和住宿费用统一由财务部门报销。")
        _add_v2_embedding(
            db, "m1", "ws", vec, provider="test_semantic", model="test_model", status="ready"
        )
    ...
    def test_dense_retrieval_stale_embeddings_excluded(self, db):
        ...
        _add_v2_embedding(
            db, "m1", "ws", vec, provider="test_semantic", model="test_model", status="stale"
        )
        retriever = DenseMemoryRetriever(db, provider)
        dense_result = retriever.search("ws", "test", limit=10)
        assert len(dense_result.candidates) == 0
    ...
    def test_workspace_isolation(self, db):
        ...
        _add_v2_embedding(
            db, "m1", "ws-a", vec, provider="test_semantic", model="test_model", status="ready"
        )
        retriever = DenseMemoryRetriever(db, provider)
...
class TestRetrievalGate:
    ...
    def test_any_message_searches(self):
        gate = RetrievalGate()
        from cogito_agent.retrieval.query import MemoryQueryContext
        ...
        for msg in (
            ...
            ctx = MemoryQueryContext(current_message=msg)
            result = gate.evaluate(ctx)
            assert result.mode == "search", f"'{msg}' returned mode={result.mode!r}"

    def test_query_enrichment_with_session_context(self):
        gate = RetrievalGate()
        from cogito_agent.retrieval.query import MemoryQueryContext
        ...
        ctx = MemoryQueryContext(
            ...
            session_topic_summary="Python学习",
        )
        result = gate.evaluate(ctx)
        assert result.mode == "search"
        # enriched query should include session context
        assert "Python" in result.enriched_query
        assert "文档" in result.enriched_query
...
class TestResidentMemorySelector:
    def test_selects_pinned_profile(self, db):
        _ensure_workspace(db, "ws")
        _add_memory(
            db, "m1", "ws", "user profile info", mtype="profile", confidence=0.9, pinned=True
        )
    ...
    def test_respects_type_policy(self, db):
        ...
        _add_memory(db, "m2", "ws", "more profile", mtype="profile", confidence=0.8, pinned=True)
        _add_memory(db, "m3", "ws", "third profile", mtype="profile", confidence=0.7, pinned=True)
        selector = ResidentMemorySelector(db)
        type_policy = {
    ...
    def test_ignores_deleted(self, db):
        _ensure_workspace(db, "ws")
        _add_memory(db, "m1", "ws", "profile info", mtype="profile", confidence=0.9, pinned=True)
        db.connection.execute("UPDATE memories SET deleted_at = datetime('now') WHERE id = 'm1'")
        db.connection.commit()
        selector = ResidentMemorySelector(db)
        results = selector.select("ws", token_budget=500)
        assert len(results) == 0
...
def test_hybrid_service_works(db):
    _ensure_workspace(db, "ws")
    _add_memory(db, "m1", "ws", "apple banana fruit", mtype="general", confidence=0.9)
    _add_memory(db, "m2", "ws", "cat dog animal", mtype="general", confidence=0.5)

    provider = _TestSemanticProvider(dimension=384)
    vec_fruit = provider.embed_text("apple banana fruit")
    _add_v2_embedding(
        db, "m1", "ws", vec_fruit, provider="test_semantic", model="test_model", status="ready"
    )
...
def test_pinned_not_dominating_low_relevance(db):
    ...
    _add_memory(
        ...
        "m1",
        "ws",
        "highly relevant content about python programming",
        mtype="general",
        confidence=0.9,
        pinned=False,
    )
    _add_memory(
        db, "m2", "ws", "completely unrelated", mtype="general", confidence=0.5, pinned=True
    )
    ...
    _add_v2_embedding(
        db, "m1", "ws", vec_m1, provider="test_semantic", model="test_model", status="ready"
    )
    _add_v2_embedding(
        db, "m2", "ws", vec_m2, provider="test_semantic", model="test_model", status="ready"
    )
    ...
    result = svc.recall(ctx, limit=10)
    # With RRF, ranking is by rank position (not weighted sum).
    # The key invariant: both memories should be included (pinned doesn't
    # exclude relevant), and the relevant one should have a reasonable score.
    ids = [str(m["id"]) for m in result.dynamic_memories]
    assert "m1" in ids, "Relevant memory should not be excluded"
    assert "m2" in ids, "Pinned memory should not be excluded either"
    m1_breakdown = result.score_breakdowns.get("m1", {})
    assert m1_breakdown.get("semantic_score", 0) > 0.1, "m1 should have non-trivial semantic score"
...
def test_resident_only_gate_mode(db):
    ...
    from cogito_agent.retrieval.service import MemoryRetrievalService as _MRS  # noqa: N814
    ...
    result = svc.recall(ctx, limit=10)
    assert result.mode in ("search", "hybrid")
    assert result.resident_count >= 0
...
def test_config_rrf_fusion(db):
    """RRF fusion ranks by reciprocal rank, not by absolute score."""
    _ensure_workspace(db, "ws")
    ...
    fusion = CandidateFusion(rrf_k=60.0, keyword_weight=0.5)

    # Two items: m1 has both dense + sparse, m2 has only sparse
    sparse_candidates = [
        {
            "id": "m1",
            "text": "python programming",
    ...
    results = fusion.fuse(sparse_candidates, dense_candidates, "python")
    assert len(results) == 2

    # m1 appears in both channels → RRF boost → should rank first
    m1_score = None
    m2_score = None
    for item, breakdown in results:
        if item["id"] == "m1":
            m1_score = breakdown.final_score
        if item["id"] == "m2":
            m2_score = breakdown.final_score

    # m1: dense-rank=1 → 1/(60+1), kw-rank=1 → 0.5/(60+1) → total ≈ 0.0246
    # m2: kw-rank=2 → 0.5/(60+2) → total ≈ 0.0081
    assert m1_score is not None and m2_score is not None
    assert m1_score > m2_score, "m1 (in both channels) should rank above m2 (sparse only)"

    # Verify RRF scores are in expected range (< 1.0 since they're rank-based)
    assert m1_score < 0.1
    assert m2_score < 0.05
...
def test_context_engine_distinguishes_resident_and_dynamic(db):
    _ensure_workspace(db, "ws")
    _add_memory(
        db, "m1", "ws", "resident profile info", mtype="profile", confidence=0.9, pinned=True
    )
    _add_memory(db, "m2", "ws", "dynamic retrieved info", mtype="general")
    provider = _TestSemanticProvider(dimension=384)
    vec = provider.embed_text("dynamic retrieved info")
    _add_v2_embedding(
        db, "m2", "ws", vec, provider="test_semantic", model="test_model", status="ready"
    )
    ...
    recall.resident_memories = [
        {
            "id": "m1",
            "text": "resident profile info",
            "retrieval_source": "resident",
            "type": "profile",
        }
    ]
    recall.dynamic_memories = [
        {
            "id": "m2",
            "text": "dynamic retrieved info",
            "retrieval_source": "dynamic",
            "type": "general",
        }
    ]
